...
export default function MarqueeDesc(props) {
  return (
    <div className='bg-primary shadow-neon-top string-light-border flex h-3/5 w-full flex-col justify-start overflow-scroll rounded-br-2xl border-b-[1rem] px-6 py-1 sm:text-xs md:text-xs lg:text-sm xl:text-[0.9rem] xl:leading-5'>
        ...
        if (props.action[index] === 'copy') {
          return (
            <div key={item} className='flex w-full border-y-[1px] border-[#00000033] px-4 py-[0.33rem] shadow-inner'>
              <button
                className='w-full cursor-pointer break-all text-stone-700 hover:text-red-600 active:cursor-copy active:text-green-700'
